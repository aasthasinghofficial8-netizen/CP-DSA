# [3.Rabbits In Forest](https://leetcode.com/problems/rabbits-in-forest/description/)
...
public:
    ...
    for(int i=1;i<1000;i++){
        if(hsh[i]<=i+1 && hsh[i]!=0){
            ans+=i+1;
        }
        else if(hsh[i]>i+1 && hsh[i]!=0){
            int n=hsh[i]/(i+1);
            int k=hsh[i]%(i+1);
            if(k==0){
                ans+=n*(i+1);
...
<p>
<ul>
<li>Logic building and optimisation</li>
</ul>
</p>
...
<p>
<ul>
<li>Logic built through test cases and submissions</li>
</ul>
</p>
